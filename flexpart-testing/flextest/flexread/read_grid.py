...
This is taken from a version of pflexible from Don Morton.
For original pflexible contact J. Bukhardt.
It is adapted so that we only have the reading of the header
for FLEXPART version 6.X, 8.X and further. Some other things have
been modified. Classes and helper functions also from pflexible

NO plotting options are included
NO FortFlex is included for simplicity

Warning: Version 9 and Version 8 reagdrig are the same
"""
...
#        a=[struct.unpack(ft,f.read(struct.calcsize(ft))) for ft in Dfmt]
        a = f2.read(fmt, n)
        ### DJM HACK - 2013-09-02 - without the following, 'a' ends up being
        ### a float rather than a numpy array
        if n==1:
            a = np.array([a]) 
#        dumplist=[a[j][0] for j in range(len(a))]
    ...
    def _dumpgrid(dmp_i, cnt_r, dmp_r, grd, k, nage, nx, ny):
        ...
        print 'TYPE DMP_R: ' + str(type(dmp_r))
        '''
