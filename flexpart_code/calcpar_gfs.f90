...
subroutine calcpar_gfs(n,uuh,vvh,pvh)
  ...
  ! ttlev 
  ! qvlev
  ! * obukhov_gfs      subroutine/function to calculate Obukhov length *
  ...
  real :: ttlev(nuvzmax),qvlev(nuvzmax),obukhov_gfs,scalev,ol,hmixplus
 ...
 ! Other variables:
  ...
  integer :: ix,jy,i,kz,lz,kzmin,llev
...
!-----------------------------------------------------------------------------
  ...
  ! calculate inverse Obukhov length scale with tth(llev)
      ol=obukhov_gfs(ps(ix,jy,1,n),tt2(ix,jy,1,n),td2(ix,jy,1,n), &
  ...
  ! NCEP version hmix has been read in in readwind.f, is therefore not calculated here
      call richardson_gfs(ps(ix,jy,1,n),ustar(ix,jy,1,n),ttlev,qvlev, &
